...
class MaskBoundingBoxWithTests:
    ...
    RETURN_TYPES = ("MASK", "IMAGE", "INT", "INT", "INT", "INT", "BOOLEAN")
    RETURN_NAMES = ("MASK", "IMAGE", "x", "y", "width", "height", "valid_bbox")
    FUNCTION = "execute"
    CATEGORY = "essentials/mask"
    ...
    def execute(self, mask, padding, blur, image_optional=None):
        ...
        # find bounding box safely
        coords = torch.where(mask > 0)
        if coords[0].numel() == 0:  # No mask pixels found
            # fallback to full image
            x1, y1 = 0, 0
            y2, x2 = mask.shape[1], mask.shape[2]
            valid_bbox = False
        else:
            _, y, x = coords
            x1 = max(0, x.min().item() - padding)
        ...
        return (mask, image_optional, x1, y1, x2 - x1, y2 - y1, valid_bbox)
